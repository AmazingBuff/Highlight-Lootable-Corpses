...
- 三种显示模式（`DisplayMode`）三选一：`silhouette` 按高亮物体之间的逐像素深度选择最近物体的穿墙剪影填充、
  `outline` 为每个物体绘制明亮窄核心和向外平滑衰减的彩色光晕（被其他物体遮住的边界仍可见）、`icon` 尸体顶部上方的向下箭头，空间和屏幕上都接近的尸体合并为双箭头
...
参见[功能文档索引](docs/features/README.md)，其中的 [Mask 渲染](docs/features/mask-rendering.md)说明模式语义、逐目标颜色接口、双通道光晕资源、保守局部区域处理与验证。
