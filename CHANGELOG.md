...
- Add an experimental mesh outline mask renderer behind the `OutlineMaskDebug` INI option (off by default): corpse meshes — fully GPU-skinned via per-partition bone matrices — are rendered into an offscreen mask (RGBA8, no depth test, so it penetrates walls) that the upcoming edge-detection outline pass will consume. Enabling the option overlays the mask in translucent magenta for visual verification; nothing is rendered while the option is off.
