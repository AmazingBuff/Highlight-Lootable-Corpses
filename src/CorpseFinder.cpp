...
namespace
{
	...
	[[nodiscard]] float HkX(const RE::hkVector4& a_v) { return a_v.quad.m128_f32[0]; }
	[[nodiscard]] float HkY(const RE::hkVector4& a_v) { return a_v.quad.m128_f32[1]; }
	[[nodiscard]] float HkZ(const RE::hkVector4& a_v) { return a_v.quad.m128_f32[2]; }

	[[nodiscard]] RE::NiPoint3 HkToNi(const RE::hkVector4& a_v)
	{
	...
	[[nodiscard]] RE::NiTransform HkTransformToNi(const RE::hkTransform& a_t)
	{
		RE::NiTransform out;
		out.scale = 1.0f;
		const auto& r = a_t.rotation;
		...
		const float s   = WorldScaleInverse();
		out.translate = { HkX(a_t.translation) * s, HkY(a_t.translation) * s, HkZ(a_t.translation) * s };
		return out;
	}

	void ExpandAabb(RE::NiPoint3& a_min, RE::NiPoint3& a_max, const RE::NiPoint3& a_p)
	{
	...
	[[nodiscard]] bool AddRigidBodyAabb(RE::bhkRigidBody* a_body, RE::NiPoint3& a_min, RE::NiPoint3& a_max)
	{
		...
		const float      s  = WorldScaleInverse();
		const RE::NiPoint3 mn{ HkX(aabb.min) * s, HkY(aabb.min) * s, HkZ(aabb.min) * s };
		const RE::NiPoint3 mx{ HkX(aabb.max) * s, HkY(aabb.max) * s, HkZ(aabb.max) * s };
		...
		if (auto* colObj = a_node->GetCollisionObject()) {
			if (auto* body = colObj->GetRigidBody()) {
				if (auto* rb = body->GetRigidBody()) {
					if (rb->world) {  // 在 Havok 世界里 → 变换实时有效
						if (AddRigidBodyAabb(body, a_min, a_max)) {
							++a_count;
							if (a_obbCorners) {
								const auto* shape = rb->GetShape();
								if (shape && shape->type == RE::hkpShapeType::kBox) {
									const auto* box = static_cast<const RE::hkpBoxShape*>(shape);
									const float s   = WorldScaleInverse();
									const auto  he  = HkToNi(box->halfExtents) * s;
									const float vol = he.x * he.y * he.z;
									if (vol > a_bestVolume) {
										a_bestVolume = vol;
										const auto world = HkTransformToNi(rb->motion.motionState.transform);
										for (int i = 0; i < 8; ++i) {
	...
	[[nodiscard]] bool ComputeRagdollBounds(RE::Actor* a_actor, RE::NiPoint3& a_min, RE::NiPoint3& a_max)
	{
		...
		for (const auto& graph : animGraphManager->graphs) {
			if (!graph) {
				continue;
			}
	...
	[[nodiscard]] std::vector<RE::FormID> ResolveFormIDs(std::initializer_list<StaticFormID> a_forms)
	{
		...
		if (dh) {
			for (const auto& f : a_forms) {
				const auto id = dh->LookupFormID(f.local, f.plugin);
				if (id != 0) {
					out.push_back(id);
				}
			}
	...
	[[nodiscard]] const std::vector<RE::FormID>& AshPileFormIDs()
	{
		static const auto ids = ResolveFormIDs({
	...
	// 不依赖关联 Actor。
	[[nodiscard]] const std::vector<RE::FormID>& CorpseObjectFormIDs()
	{
		static const auto ids = ResolveFormIDs({
			// Skyrim.esm
	...
	[[nodiscard]] RE::Actor* FindAshPileOwner(RE::TESObjectREFR* a_pile)
	{
		...
		const auto pileLink = a_pile->extraList.GetAshPileRef();
		if (pileLink) {
		...
		const auto pileHandle = a_pile->GetHandle().native_handle();
		auto*      processLists = RE::ProcessLists::GetSingleton();
		if (!processLists) {
			return nullptr;
		}
		const auto findIn = [&](const RE::BSTArray<RE::ActorHandle>& a_list) -> RE::Actor* {
...
namespace CorpseFinder
{
	void Scan()
	{
		...
		const auto consider = [&](RE::Actor* a_actor) {
			...
			const bool   ragdoll = a_actor->IsInRagdollState();
		...
		const auto considerObject = [&](RE::TESObjectREFR* a_ref) {
			const bool isAsh       = IsAshPileRef(a_ref);
			const bool isCorpseObj = isAsh ? false : IsCorpseObjectRef(a_ref);
			if (!isAsh && !isCorpseObj) {
				return;
			}

			// 诊断：只读检查容器状态（不创建任何东西）
			const bool      hasExtra = a_ref->extraList.HasType<RE::ExtraContainerChanges>();
			auto*          changes  = a_ref->GetInventoryChanges(true);
			const auto     entryCount = changes && changes->entryList ? changes->entryList->size() : 0;
			// 引擎自身视角（地址库重定位，非虚表）：容器 UI 用的条目计数。
			// 参数变体都试一遍，避免语义/调用方式偏差；任何一个 > 0 都视为可搜刮。
			const auto     cntView = std::max(a_ref->GetInventoryItemCount(true, false), 0);
			const auto     cntSelf = std::max(a_ref->GetInventoryItemCount(false, false), 0);
			const auto     cntPlay = std::max(a_ref->GetInventoryItemCount(false, true), 0);
			const auto     ashLink = a_ref->extraList.GetAshPileRef().native_handle();
			const bool     baseLoot = isCorpseObj && HasContainerLoot(a_ref);
		...
		for (const auto& g_corpse : g_corpses)
		{
			auto dh = RE::TESDataHandler::GetSingleton();
			if (!dh) {
				break;
