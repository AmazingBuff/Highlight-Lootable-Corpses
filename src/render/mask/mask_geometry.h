...
// Diagnostic logging for the skinned path's give-up/failure exits - emitted once per (node name,
// reason) signature (with values attached); the same signature is not repeated. Shared inside the
// mask subsystem.
