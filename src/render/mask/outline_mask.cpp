...
namespace
{
        ...
        ID3D11RenderTargetView* overlay_target, std::uint32_t width, std::uint32_t height)
    {
        ...
        std::vector<Mask::MaskTarget> const& targets = g_targets;
        ...
        if (!g_geometry_pass.init(device))
            return;
        if (silhouette ? !g_silhouette_pass.init(device) : !g_outline_pass.init(device))
            return;
        ...
        Mask::FullscreenPass& consumer = silhouette ? static_cast<Mask::FullscreenPass&>(g_silhouette_pass)
                                                    : static_cast<Mask::FullscreenPass&>(g_outline_pass);
        ...
        auto const draw_mask = [&](std::span<Mask::MaskDraw const> group) {
            ...
            context->OMSetRenderTargets(1, &mask_rtv, silhouette ? g_mask_rt.dsv() : nullptr);
            context->ClearRenderTargetView(mask_rtv, Mask::Mask_Clear_Color);
            if (silhouette)
                context->ClearDepthStencilView(g_mask_rt.dsv(), D3D11_CLEAR_DEPTH, 0.0f, 0);
            context->OMSetBlendState(g_geometry_pass.mask_write_blend(), nullptr, 0xFFFFFFFF);
            context->OMSetDepthStencilState(silhouette ? g_geometry_pass.depth_nearest() : g_geometry_pass.depth_none(), 0);
            context->RSSetState(g_geometry_pass.cull_none());
            context->RSSetViewports(1, &vp);
            g_geometry_pass.draw(device, context, view_proj, group);
        };
        if (silhouette)
        {
            draw_mask(draws);
            g_silhouette_pass.draw(context, overlay_target, g_mask_rt.srv(), width, height,
        ...
        else
        {
            ...
            for (std::size_t begin = 0; begin < draws.size();)
            {
                ...
                if (!g_outline_pass.draw(context, overlay_target, g_mask_rt.srv(), width, height, cfg.outline_thickness,
                        g_geometry_pass.depth_none(), g_geometry_pass.cull_none()))
                    break;
                begin = end;
            }
