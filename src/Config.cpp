...
namespace Config
{
	std::filesystem::path GetIniPath() noexcept
	{
		...
		const auto exePath = REL::Module::get().filePath();  // SkyrimSE.exe 的完整路径（wstring_view）
	...
	namespace
	{
		std::uint32_t ParseHex(const char* a_value, std::uint32_t a_default) noexcept
		{
			if (!a_value || !*a_value) {
				return a_default;
			}
			char* end = nullptr;
			const auto value = std::strtoul(a_value, &end, 16);
