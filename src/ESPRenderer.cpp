...
namespace
{
	...
	bool EnsureBackBuffer(IDXGISwapChain* a_swapChain, ID3D11Device* a_device)
	{
		...
		const HRESULT hr = a_swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&buffer));
		if (FAILED(hr) || !buffer) {
			return false;
		}
		...
		g_backW = desc.Width;
		g_backH = desc.Height;
		const HRESULT rtvHr = a_device->CreateRenderTargetView(buffer, nullptr, &g_backBufferRTV);
	...
	void DrawFilledRect(float a_x0, float a_y0, float a_x1, float a_y1, const DirectX::XMFLOAT4& a_color)
	{
		const DirectX::VertexPositionColor vertices[6] = {
	...
	void DrawRectOutline(float a_x0, float a_y0, float a_x1, float a_y1, float a_thickness, const DirectX::XMFLOAT4& a_color)
	{
		const float t = std::max(a_thickness, 1.0f);
	...
	void DrawThickLine(const DirectX::XMFLOAT2& a_p0, const DirectX::XMFLOAT2& a_p1, float a_thickness, const DirectX::XMFLOAT4& a_color)
	{
		const float t = std::max(a_thickness, 1.0f) * 0.5f;
		const float dx = a_p1.x - a_p0.x;
		const float dy = a_p1.y - a_p0.y;
		const float len = std::sqrt(dx * dx + dy * dy);
		if (len < 1e-4f) {
			DrawFilledRect(a_p0.x - t, a_p0.y - t, a_p0.x + t, a_p0.y + t, a_color);
			return;
		}
		const float nx = -dy / len * t;
		const float ny = dx / len * t;
		const DirectX::VertexPositionColor vertices[6] = {
	...
	void OnPresent(IDXGISwapChain* a_swapChain)
	{
		...
		if (Config::Get().showIndicator) {
			...
			const auto color = enabled
				? DirectX::XMFLOAT4{ 0.0f, 1.0f, 0.35f, 1.0f }
				: DirectX::XMFLOAT4{ 0.45f, 0.45f, 0.45f, 0.8f };
		...
		if (Config::IsEnabled()) {
			...
			const bool drawSomething = cfg.showOutline || cfg.showGlow || cfg.showCenterDot;
			if (drawSomething) {
				...
				const RE::BSGraphics::ViewData* viewData = nullptr;
				if (state) {
					auto& stateRt = state->GetRuntimeData();
					for (const auto& camData : stateRt.cameraDataCacheA) {
				...
				const auto rgb = cfg.outlineColor;
				const float cr = static_cast<float>((rgb >> 16) & 0xFF) / 255.0f;
				...
				for (const auto& corpse : CorpseFinder::Snapshot()) {
					...
					auto projectToScreen = [&](const RE::NiPoint3& a_pt, float& a_px, float& a_py, float& a_depth) -> bool {
						...
						if (worldCam && worldCam->WorldPtToScreenPt3(a_pt, a_px, a_py, a_depth, 1e-5f)) {
							...
							const auto& viewProj = viewData->viewProjMatrixUnjittered._11 != 0.0f
								? viewData->viewProjMatrixUnjittered
								: viewData->viewProjMat;
							...
							if (std::fabs(clipW) >= 1e-5f) {
								const DirectX::XMVECTOR ndc = DirectX::XMVectorDivide(clip, DirectX::XMVectorReplicate(clipW));
								const float ndcX = DirectX::XMVectorGetX(ndc);
								const float ndcY = DirectX::XMVectorGetY(ndc);
								const float ndcZ = DirectX::XMVectorGetZ(ndc);
					...
					auto addProjected = [&](const RE::NiPoint3& a_pt, int a_idx = -1) {
					...
					// 矩形与中心
					const float sx = (minX + maxX) * 0.5f;
					const float sy = (minY + maxY) * 0.5f;
					const float boxW = maxX - minX;
					const float boxH = maxY - minY;
					//const float sz = depthCount > 0 ? depthSum / static_cast<float>(depthCount) : 0.0f;

					// 保证最小可见尺寸（太远时不会缩成一个点）
					constexpr float kMinBox = 5.0f;
					float x0 = minX, y0 = minY, x1 = maxX, y1 = maxY;
					if (boxW < kMinBox || boxH < kMinBox) {
						const float halfX = std::max(boxW * 0.5f, kMinBox * 0.5f);
						const float halfY = std::max(boxH * 0.5f, kMinBox * 0.5f);
						x0 = sx - halfX;
						x1 = sx + halfX;
						y0 = sy - halfY;
						y1 = sy + halfY;
					...
					const float half = radiusPx * 1.15f;
						...
						if (cfg.showGlow) {
							...
							for (int i = 4; i >= 1; --i) {
								const float grow = half * (1.0f + 0.25f * static_cast<float>(i));
								const float ga = alpha * cfg.glowAlpha / static_cast<float>(i);
								DrawFilledRect(
						...
						if (cfg.showOutline) {
							if (useWireframe) {
								...
								const auto col = DirectX::XMFLOAT4{ cr, cg, cb, alpha };
						...
						if (cfg.showCenterDot) {
							const float d = std::max(2.0f, radiusPx * 0.12f);
							DrawFilledRect(
								sx - d, sy - d, sx + d, sy + d,
