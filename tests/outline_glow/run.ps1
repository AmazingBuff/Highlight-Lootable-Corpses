...
$buildPath = Join-Path $Repository 'build/outline_glow'
...
@echo off
$setup
cl.exe /nologo /std:c++20 /EHsc /W4 /utf-8 /DNOMINMAX /I"$Repository/src" "$PSScriptRoot/smoke.cpp" /Fe:outline_glow_smoke.exe /link d3d11.lib d3dcompiler.lib
if errorlevel 1 exit /b 1
outline_glow_smoke.exe "$Repository" "$buildPath"
exit /b %errorlevel%
"@
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Outline glow smoke failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}
